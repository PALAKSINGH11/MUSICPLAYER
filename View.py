...
class View:
    ...
    def setup_player(self):
           ...
           self.top.title("MOUZIKKA-Let Music flow in your heart and enrich your soul!!")
    ...
    def add_song_to_favourites(self):
        self.fav_song_index_tuple = self.playList.curselection()
        try:
            if len(self.fav_song_index_tuple) == 0:
                raise NoSongSelectedError("Please select a song first")
            self.song_name = self.playList.get(self.fav_song_index_tuple[0])
            result = self.my_player.add_song_to_favourites(self.song_name)
            ...
            messagebox.showerror("DB Error!!!", "Song can't be Added")
            print(traceback.format_exc())
    ...
    def load_songs_from_favourites(self):
        try:
            result, song_dict_myfav = self.my_player.load_songs_from_favourites()
            if result.find("No songs present") != -1:
                messagebox.showinfo("Favourites Empty!!!", "No song in your Favourites")
                return
            self.playList.delete(0, tk.END)
            self.stop_song()
            for song_name in song_dict_myfav:
                self.playList.insert(tk.END, song_name)
            rcolor = lambda: random.randint(0, 255)
            red = hex(rcolor())
            green = hex(rcolor())
    ...
    def remove_song_from_favourites(self):
        ...
        try:
            if len(self.fav_song_index_tuple) == 0:
                raise NoSongSelectedError("Please select a song first")
            self.song_name = self.playList.get(self.fav_song_index_tuple[0])
            # self.playList.delete(self.fav_song_index_tuple[0])
            self.stop_song()
            self.playList.delete(self.fav_song_index_tuple[0])
